    ...
    return (
        <React.Fragment>
            ...
            <LayoutApp>
                <div
                    className={`${
                        darkMode ? "bg-darkText" : "bg-lightBg"
                    }  bg-homeBg bg-cover bg-center mt-[150px] mb-[50px]`}
                >
                    <div class="max-w-screen-xl mx-auto p-5 sm:p-8 md:p-12 relativ mt-[100px]">
                        <div
                            class={`max-w-3xl mx-auto -mt-[50px] ${
                                darkMode
                                    ? "bg-darkText text-lightText"
                                    : "bg-lightBg text-darkText"
                            } rounded-xl relative z-11  `}
                        >
                            <div class=" p-10 shadow-xl shadow-purple-900  rounded-xl flex flex-col justify-between leading-normal relative z-11  ">
                                <div className="flex flex-row items-center align-middle gap-3">
                                    <div className="h-20 w-20 shrink-0 grow-0 rounded-full relative">
                                    <img
                                        src={"/imageusers/" + user?.imagePath}
                                        className="h-full w-full shrink-0 grow-0 rounded-[100%]"
                                        alt=""
                                    />
                                    {isOnLine && (
                                        <div className="bg-green-500 h-5 w-5 rounded-full absolute bottom-0 right-0"></div>
                                    )}
                                    </div>
                                    ...
                                    <div className="w-full flex flex-row justify-between items-center">
                                        ...
                                        <div>
                                            <span className={`${user.is_admin == 0 ? "bg-lightText text-darkText border-[1px] border-darkText" : user.is_admin == 1 ? "bg-darkText text-lightText border-[1px] border-lightText" : null} p-1 rounded-md`}>{user.is_admin == 1  ? "Admin" : user.is_admin == 0 ? "User" : null}</span>
                                        </div>
                                ...
                                {/* <div class=""> */}
                                {/* <img src={"/imageusers/"+user?.imagePath} className='h-20 w-20 rounded-full' alt="" />

                <h1 href="#" class=" font-bold text-3xl mb-2">{user.name}</h1> */}
                ...
                {/* {user.posts?.map((post,i) => (
                     <div  className='hover:rounded-xl shadow-xl shadow-purple-900 group hover:scale-105 duration-300'>
                         ...
                         <div className='flex flex-row justify-between items-center'>
                             <Link href={`/user/${user?.id}`} className='font-bold text-sm'>{user.name}</Link>
                    ...
                    <BlogCard
                        title={post.title}
                        username={user.name}
                        postHref={`/blogs/${post.id}`}
                        userHref={`/user/${user?.id}`}
                        description={post.description.substring(0, 70)+ ' ...'}
                        postTags={post?.tags}
                        created_at={moment(post.created_at).fromNow()}
                        imagePath={"/imageusers/"+post.imagePath}
                    />
                ))} */}
                <Pagination Component={BlogCard} postTags={user.posts?.map(post => post.tags?.map(tag => tag))} tagId={user.posts?.map(post => post.tags?.map(tag => tag.id))} userItem={user} items={user.posts}  />
            </div>
                    </div>
                </div>
            </LayoutApp>
